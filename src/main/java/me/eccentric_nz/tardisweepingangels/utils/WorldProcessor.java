	...
	public void run() {
		if (!config.contains("config_version")) {
			...
			File newFile = new File(
					plugin.getDataFolder() + File.separator + "config_" + System.currentTimeMillis() + ".yml");
		...
		plugin.getServer().getWorlds().forEach((w) -> {
			...
			if (!config.contains("ood.worlds." + n) ||
				(config.contains("ood.worlds." + n) && config.getInt("ood.worlds." + n) == 20)) {
				plugin.getConfig().set("ood.worlds." + n, true);
			}
			if (!config.contains("judoon.worlds." + n) || (config.contains("judoon.worlds." + n) &&
														   Objects.equals(config.getString(
																   "judoon.worlds." + n), "true"))) {
